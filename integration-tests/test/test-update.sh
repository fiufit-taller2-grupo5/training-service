...
# Step 1: Execute sudo docker compose up for running the db
...
while ! sudo docker exec -e PGPASSWORD="12345678" -it "postgres-service" psql -h "localhost" -p "5432" -U "postgres" -w -d "postgres-test" -c "SELECT 1" > /dev/null 2>&1; do
  sleep 1
done
...
cd ../../../user-service/database/prisma/prisma
yarn && npx prisma db push

# Step 4: Stop the database
cd ../../../../development-setup/integration-tests/test
